...
class Calculator extends React.Component {
  ...
  handlechange(event) {
    ...
    if (id === 'DEL') {
      ...
      const resultdisplay = Math.round(result * 100) / 100
      

      this.setState({
        //rounding up the result to 2 decimal points
        display: resultdisplay,
        calculate: resultdisplay.toString().split('')
      })

      console.log("display: " + this.state.display, "calculate: "+ this.state.calculate)
    }
    else if (id === 'x') {
      this.setState({
        display: num + '*'
